...
<head>
  ...
  function Init() {
	...
	if (a_prop.canchor.options.length<=1)
	{
		...
		a_prop.catype.options.remove(3);
	}
	changeType(atype);    
  }
    ...
    if (validateParams())    
    {
      var aProps = {};
		  if (a_prop.catype.options[a_prop.catype.selectedIndex].value == "link2anchor")
			...
			else if (a_prop.catype.options[a_prop.catype.selectedIndex].value == "internal")
				aProps.href = a_prop.ilink.options[a_prop.ilink.selectedIndex].value;
		  else
...
// line 106
  function changeType(new_type)
  {
	...
	else if (new_type == "link2anchor")
	{
  	...
  	target_row.style.display = (new_type=="link"||new_type=="link2anchor"||new_type=="internal")?"inline":"none";
		internal_row.style.display = (new_type=="internal")?"inline":"none";
		title_row.style.display = (new_type=="internal")?"none":"inline";
...
<tr id="internal_row">
	<td>Page</td>
	<td><select name="ilink" class="input">
<?
	include $internal_link_file;
?>
</select></td>
</tr>
<tr>
