...
function getSentStat($fid, $yr, $quart){
	$query = "SELECT UNIX_TIMESTAMP(paiddate) AS paiddate , percentage FROM fundraisers_payments WHERE fid=".$fid." AND year=".$yr." AND quarter=".$quart;
	...
	if(mysql_num_rows($result)>0){
		...
		return array($timestamp, $percentage);
	}else{
		return false;
	}
}
...
function getCommissionRecordsSummary($code, $startdate, $enddate){
	...
	." AND b.finished > '". date('Y-m-d', $startdate)."' AND b.finished < '" . date('Y-m-d', $enddate+86400) . "' AND b.status='posted' ";
	//echo $query;
	$result = mysql_query($query);
	if(!$result) error_message(sql_error());
	return $result;
...
function getCommissionProductCostFromRecSummary($result){
	while($qdata=mysql_fetch_array($result)){
	    if($qdata['price'] > 0){
		  $totalcommision+=$qdata["price"];
		}
	}
