...
<link href="css/identi kid.css" rel="stylesheet" type="text/css" />
		...
		if(mysql_num_rows($result)>0){
			while($qdata = mysql_fetch_array($result)){
//				debug_showvar($qdata);
			$qdata["text1"] = stripslashes($qdata["text1"]);
			$qdata["text2"] = stripslashes($qdata["text2"]);
			$qdata["text3"] = stripslashes($qdata["text3"]);
			$qdata["text4"] = stripslashes($qdata["text4"]);
			$qdata["text5"] = stripslashes($qdata["text5"]);
		?>
			<tr> 
				<td>
						...
						<tr>
						  ...
						  if($qdata["type"]==7){
								...
								if($qdata["type"]==6 || $qdata["type"]==11 && $qdata["basketid"] < 63006  ){ //61266 mixed pack
									...
									<?	
								}else if(($qdata["type"]>=48 && $qdata["type"]<=72)||($qdata["type"]>=80 && $qdata["type"]<=82)){
								?>
							...
							<tr>
								...
								$width = "300";
									...
									<OBJECT classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"
									 ...
									 <PARAM NAME=quality VALUE=high>
									 <PARAM NAME=bgcolor VALUE=#FFFFFF>
									 <EMBED src="<? echo $aim;?>images/display_bin_labels.swf<? echo $swfstring;?>" quality=high bgcolor=#FFFFFF  WIDTH="<? echo $width;?>" HEIGHT="<? echo $height;?>" NAME="display_bin_labels" ALIGN=""
									 TYPE="application/x-shockwave-flash" PLUGINSPAGE="<? echo $pluginspace;?>"></EMBED>
									</OBJECT>
									<?	
								}
								...
								// 16 Baby Pack
									...
									if ( $qdata["basketid"] > 0){
										...
										<OBJECT classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"
										 ...
										 <PARAM NAME=quality VALUE=high>
										 <PARAM NAME=bgcolor VALUE=#FFFFFF>
										 <EMBED src="<? echo $aim;?>images/display_mini.swf<? echo $swfstring;?>" quality=high bgcolor=#FFFFFF  WIDTH="<? echo $width;?>" HEIGHT="<? echo $height;?>" NAME="display_mini_new" ALIGN=""
										 TYPE="application/x-shockwave-flash" PLUGINSPAGE="<? echo $pluginspace;?>"></EMBED>
										</OBJECT>
										<?
									}
...
/*
				if($from=="user"){
					...
					elseif($qdata["type"]==15){
					  $filename ="Products/Gift_Vouchers/Mail_Only";
					}
					elseif($qdata['type']==16){
						$filename="Products/Packs/Baby_Pack";
					}
