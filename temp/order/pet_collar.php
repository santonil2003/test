...
function update() { 
  letterValid = /[A-Za-z]/;
  notValid = /[^A-Za-z ]/;
  collarType = getCheckedValue(document.form1.collar);
  ...
  document.getElementById('typedetail').value = collarType=="Dog"?'0':'1';
...
function updateCost(){
  ...
  if(collarType=='Dog') {
    ...
    totalCost = (((totalLetters*letterCost)+(totalIcons*iconCost))*totalBands)+(catCollarCost*totalBands);
  }
  var quantDesc = currencySymbol+totalCost.toFixed(2);
  document.getElementById('total_cost').innerHTML = quantDesc;
  document.getElementById('quantdesc').value = "1 "+collarType+" collar For "+quantDesc;
...
<input type="hidden" name="typedetail" id="typedetail" value="0" />
...
<td class="maintext"><p>
  ...
  Cat <input class="maintext" type="radio" name="collar" id="cat" value="Cat" onclick="update();" /></p>
</td>
</tr>
<tr>
