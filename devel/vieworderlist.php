							...
							<? 
									...
									<table width="100%" align="left">
										...
										<tr>
											...
											<td class="maintext" nowrap><img src = "<?=$aim?>images/identibands/<?=$text7[4]?>.gif" border="0"></td>
										</tr>
									</table>
									</td></tr>
									<tr><td colspan="2">&nbsp;</td></tr>
									...
									<table width="100%" align="left">
										...
										<tr>
											<td class="maintext" nowrap>&nbsp;</td>
											<td class="maintext" nowrap><img src = "<?=$aim?>images/ziptags/<?=getZipTagNumber($text7[4])?>.gif" border="0"></td>
										</tr>
									</table>
									</td></tr>
									</table>
								...
								<?					
										if ($qdata["type"]==12 && $qdata["text3"] == 1){ //semi permanent iron on
											$width = "180";
											$height = "54";
											if($from=="admin"){
												$width*=2;
						 	 		...
						 	 		<?
										}elseif($qdata["type"]==12 && $qdata["text3"] == 2){ 
											$width = "180";
											$height = "54";
											if($from=="admin"){
												$width*=2;
								...
								// Book Labels
									...
									<OBJECT classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"
									 ...
									 <PARAM NAME=quality VALUE=high>
									 <PARAM NAME=bgcolor VALUE=#FFFFFF>
									 <EMBED src="<? echo $aim;?>images/display_book_labels.swf<? echo $swfstring;?>" quality=high bgcolor=#FFFFFF  WIDTH="<? echo $width;?>" HEIGHT="<? echo $height;?>" NAME="display_vinyl" ALIGN=""
									 TYPE="application/x-shockwave-flash" PLUGINSPAGE="<? echo $pluginspace;?>"></EMBED>
									</OBJECT>
									<?
								...
								//SHARED PACKS
									...
									for($i=0;$i<=1;$i++){
										...
										elseif($pack[$i]==3 && $qdata["basketid"] > 56508){
											...
											$swfstring = "?type=2&pic={$pic[$i]}&text1={$text1[$i]}&text2={$text2[$i]}&font={$font[$i]}&picon={$picon[$i]}&font_colour={$font_col[$i]}&colour={$colours[$i]}";
											?>
											<OBJECT classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"
											 codebase="<? echo $codebase;?>"
											 WIDTH="<? echo $width;?>" HEIGHT="<? echo $height;?>" id="display_mini_new" ALIGN="">
			...
			<tr> 
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					if((int)$qdata['type'] == 5 )
					{
						...
						?>
						<tr> 
						  <td width="9%" rowspan="11" class="maintext"><img src="<? echo $aim;?>images/spacer_trans.gif" width="25" height="10"></td>
						  <td class="maintext"><img src="<? echo $aim;?>images/spacer_trans.gif" width="10" height="10"></td>
						  <td width="3%" rowspan="11"><img src="<? echo $aim;?>images/spacer_trans.gif" width="10" height="10"></td>
						  <td class="maintext"><img src="<? echo $aim;?>images/spacer_trans.gif" width="10" height="10"></td>
						</tr>
						<tr> 
...
//						debug_showvar($qdata);
						...
						<tr> 
						  ...
						  <td class="maintext"><?php
								 ...
								 $tagrow = mysql_fetch_assoc($tagresult);
								  echo "<img src = 'http://www.identikid.com.au/images/identitags/".$tagrow['data_identitag_code'].".gif'>"?></td>
						</tr>
						<tr>
			...
			<?	}
					...
					<? if($qdata["type"]!=34 && !( $qdata["type"]>=36 && $qdata["type"]<=41) ){ ?>
						<tr> 
						  <td width="22%" class="maintext"><strong>Product:</strong></td>
						  <td width="66%" class="maintext"><strong><? echo getLabelType($qdata["type"]);?></strong></td>
						</tr>
						<?
						 ...
						 if($qdata["type"]!=13 && !( $qdata["type"]>=36 && $qdata["type"]<=41) && $qdata["type"]!=14 && $qdata["type"]!="15" && $qdata['type']!=18 && $qdata['type']!=19 && $qdata['type']!=20 && $qdata['type']!=28 && $qdata['type']!=29 && $qdata['type']!=33 && $qdata['type']!=34){
							?>
							<tr> 
						...
						if($qdata["type"]!=7 && !( $qdata["type"]>=36 && $qdata["type"]<=41) && $qdata["type"]!=13 && $qdata["type"]!=14 && $qdata["type"]!="15" && $qdata['type']!=18 && $qdata['type']!=16 && $qdata['type']!=33 && $qdata['type']!=34)
						{
							?>
							<tr> 
							  <td class="maintext">Pic:</td>
							...
							<tr> 
							  ...
							  <td class="maintext"><? echo getRealFontNumber($qdata["font"]);?></td>
								...
								<tr> 
								  <td class="maintext">Font Colour:</td>
								  <td class="maintext"><?php if ($qdata['data_font_colour_id']==1) echo "Black"; else echo "White"; ?></td>
								</tr>
								<tr> 
			  ...
			  <? }
				...
				<? 	if ($qdata['typedetail'] == 2) 
					...
					<?php // New Baby Packs and Starter packs
			...
			<? 	} 
				...
				if($from=="user"){
					if($qdata["type"]==1){
						...
						$filename="products_identitags.php";
					}
					...
					elseif($qdata['type']==28 || $qdata['type']==29){
						$filename = "products_zipdedo.php";
					}
					elseif($qdata['type']==30 || $qdata['type']==31){
						$filename = "products_identibands.php";
					}						
					elseif($qdata['type']==33){
						$filename = "products_book_labels.php";
					}						
					elseif($qdata['type']==34){
						$filename = "products_maxi_pack.php";
					}
					elseif($qdata['type']==35){
						$filename = "products_itty_bitty_pack.php";
					}
					elseif($qdata['type']==36){
						$filename = "products_thingamejig.php";
					}	
					elseif($qdata['type']==37){
						$filename = "products_thingamejig_collar.php";
					}	
				   elseif($qdata['type']==38){
						$filename = "products_thingamejig_boybandz.php";
					}
					elseif($qdata['type']==39){
						$filename = "products_thingamejig_gadget.php";
					}	
					elseif($qdata['type']==41){
						$filename = "products_magpie_eyes.php";
					}			
					else {
