...
if(isset($_GET["searchId"]) && $_GET["searchId"]!='') $_SESSION["searchId"] = $_GET["searchId"];
if(isset($_GET["searchName"]) && $_GET["searchName"]!='') $_SESSION["searchName"] = $_GET["searchName"];
if(isset($_GET["orderby"]) && $_GET["orderby"]!='') $_SESSION["orderby"] = $_GET["orderby"];
if(isset($_GET["orderway"]) && $_GET["orderway"]!='') $_SESSION["orderway"] = $_GET["orderway"];
...
if($fundraisershow=="Live"){
	...
	$query .= " WHERE status='offline'";
}else if($_SESSION["searchId"]!=""){
	$query .=" WHERE idnumber=".$_SESSION["searchId"];
}else if($_SESSION["searchName"]!=""){
	$query .=" WHERE fname LIKE '%".$_SESSION["searchName"]."%'";
}


if($_SESSION["orderby"]=="number"){
	$query .= " ORDER BY idnumber";
}else{
	$query .= " ORDER BY fname";
}

if($_SESSION["orderway"]=="ASC"){
	$newway = "DESC";
	$query .= " ASC";
}else{
	$newway = "ASC";
...
function postSelections(selectedAr){
  ...
  for(i=0; i<selectedAr.length; i++){
    ...
    if((postSelections.length+selectedAr[i].length+1) >= 2083 ){
      postStrings[numStrings] = postSelections;
	   numStrings++;
	   postSelections = "";
	   first=true;
    }
  ...
  if(numSent==numStrings) return true;
  else return false;
  
}

function postSelectionAjax(){

  var dataStr = "ids="+postStrings[numSent] 
  if(numSent==0) {
    dataStr+= "&reset_ids=1";
  }
  ...
  $.ajax({
    type: "POST",
    url: "fundraisers_set_ids.php",
    data: dataStr,
    async: false,
    success: function(msg){  
      numSent++;  
      if(numSent < numStrings) {
        postSelectionAjax();
      }
    }
  });
...
function viewQuarter(){
   ...
   selectedAr = getSelected();
		...
		thisF.year2[thisF.year2.selectedIndex].value == thisF.year[thisF.year.selectedIndex].value)
	{
	   ...
	   //var selections = makePostGetSelections(selectedAr);
		//document.send.ids.value = selections[1];
		var posted = postSelections(selectedAr);
		if(posted==true){
	     ...
	     document.send.action = 'fundraisers_showreports.php?set_ids=1&_gt='+greater+'&startDay='+startDay+'&startMonth='+startMonth+'&startYear='+startYear+'&endDay='+endDay+'&endMonth='+endMonth+'&endYear='+endYear+'&type=quarter'+Q2String;
		  document.send.submit();
		} else {
		  alert("There was an error setting the fundraiser ids");
		}
	}
  }
}
...
function viewDefined(){
  ...
  if(selectedAr.length==0){
	...
	if(endDate<startDate){
	  ...
	  var posted = postSelections(selectedAr);
	  if(posted==true){
	    document.send.action = 'fundraisers_showreports.php?set_ids=1&_gt='+greater+'&startDay='+startDay+'&startMonth='+startMonth+'&startYear='+startYear+'&endDay='+endDay+'&endMonth='+endMonth+'&endYear='+endYear+'&type=defined';
		 document.send.submit();
	  } else {
	    alert("There was an error setting the fundraiser ids");
	  }
	   ...
	   ?>
	     <tr>
			  <td valign="top" align="center">
			    <font color="#e51010"><?=$_GET['success']?></font>
			  </td>
		  </tr>
	   <?
	   }
		...
		<tr>
			...
			<table cellpadding="0" cellspacing="0" border="0" bgcolor="#5D7EB9">
				...
				<tr>
					 ...
					 <input type="button" name="back" value="&lt; Back" onClick="location.href='../../admin/index.php'">
				...
				<tr>
					 <td><img src="../images/spacer_trans.gif" height="1" width="1" border="0"></td>
					 <td align="center" class="admintext">
					 Search by: Id Number <input type="text" name="searchId" value="<?=$_SESSION["searchId"]?>" >&nbsp;&nbsp;Name <input type="text" name="searchName" value="<?=$_SESSION["searchName"]?>">
					 ...
					 <? if($_SESSION["searchId"]!="" || $_SESSION["searchName"]!=""){?><input type="button" value="show all" onClick="location.href='fundraisers.php?showAll=1'"><? }?></td>
				</tr>
				<tr>
					 <td><img src="../images/spacer_trans.gif" height="10" width="1" border="0"></td>
				</tr>
					 ...
					 <td colspan="3">
						 <table cellpadding="0" cellspacing="0" border="0" width="100%">
						 	<tr bgcolor="#FFFFFF">
								...
								<td><img src="../images/spacofflineer_trans.gif" height="1" width="10" border="0"></td>
							</tr>
							<tr>
							...
							<tr class="admintext">
								 ...
								 <select name="stat<? echo $qdata["id"];?>" onChange="changeStatus(<? echo $qdata["id"];?>);">
								 	<option value="live" <?=$qdata["STATUS"]=="live"?"selected":"";?> >live</option>
									<option value="offline" <?=$qdata["STATUS"]=="offline"?"selected":"";?> >offline</option>
								 </select></td>
								 <td></td>
					 ...
					 <td colspan="3" align="left">
						 <table cellpadding="0" cellspacing="0" border="0">
						 	<tr class="admintext">
								...
								<td><img src="../images/spacer_trans.gif" height="1" width="10" border="0"></td>
								<td align="right">
								All <input type="radio" name="gtq" id="gtq" value="false" checked> 
								Greater than $0.00 <input type="radio" name="gtq" id="gtq" value="0.00"> 
								Greater than $20.00 <input type="radio" name="gtq" id="gtq" value="20.00">
								<input type="button" value="Show Report" onClick="viewQuarter();"></td>
							</tr>
						 </table>
					</td>
				</tr>
					 ...
					 <td colspan="3">
						 <table cellpadding="0" cellspacing="0" border="0">
						 	<tr class="admintext">
								...
								<td><img src="../images/spacer_trans.gif" height="1" width="10" border="0"></td>
								<td align="right">All <input type="radio" name="gt" id="gt" value="false" checked> Greater than $0.00 <input type="radio" name="gt" value="0"> Greater than $20.00 <input type="radio" name="gt" value="20"> <input type="button" value="Show Report" onClick="viewDefined();"></td>
							</tr>
						 </table>
					</td>
				</tr>
