<?
 ...
 $base_url = "http://www.identikid.com.au";
...
?>
  ...
  // prevent from executing twice on the same editor
     ...
     if (this[editor+'_rEdit'].document.readyState == 'complete')
     {
	   ...
	   var linkTag = this[editor+'_rEdit'].document.createElement ("link");
            linkTag.href = css_stylesheet;
            linkTag.rel = "stylesheet";
            var head = this[editor+'_rEdit'].document.getElementsByTagName ("head")[0];
...
SPAW_toggle_borders(editor,this[editor+'_rEdit'].document.body,null);
  ...
  function SPAW_map_click(editor,curcolor) {
    var map = window.showModalDialog('<?php echo $spaw_dir?>dialogs/edit_maps_iframe.php', curcolor, 
      'dialogHeight:500px; dialogWidth:295px; resizable:no; status:no; scroll:no');  
      
    if(map != null) {   
      insertCode(this[editor+'_rEdit'], map)
    }
    	  
    SPAW_update_toolbar(editor, true);   
  }
  
  function insertCode(editor, code)
  {
    ...
    totalString = editor.document.body.innerHTML
    selectedString = editor.document.selection.createRange().text;
    if(selectedString==null || selectedString==''){
    
      editor.document.body.innerHTML = totalString + code;
    
    } else {
    
      totalLength = editor.document.body.innerHTML.length;
      selectedLength = editor.document.selection.createRange().text.length;
      ...
      editor.document.body.innerHTML = firstTextValue + code + lastTextValue;
		...
		else
		{
		   ...
		   */
		    if (selection.htmlText == '')
			  ...
			  var strInputCode = selection.htmlText;
			  var strTagStrippedText = strInputCode.replace(/<\/?[p|div|dd|center|menu|ol|li]+(>|$)/gi, "");
			  a.innerHTML = strTagStrippedText; //a.innerHTML = selection.htmlText;
			 }
...
//Mindaugas Griunas; insert_doc

  function SPAW_doc_click(editor, sender)
  ...
  {	var nProps = {};
    ...
    window.frames[editor+'_rEdit'].focus(); 
  	...
  	if (window.frames[editor+'_rEdit'].document.selection.type == "Control") {
			...
			var lControl = window.frames[editor+'_rEdit'].document.selection.createRange();

			if (lControl(0).tagName.toUpperCase() == "IMG") {

				var lSel = lControl(0).parentNode;
			...
			lSel = window.frames[editor+'_rEdit'].document.selection.createRange().parentElement();

		}

		if (lSel.tagName.toUpperCase() == "A")

		{

			nProps.target = lSel.target

			nProps.link = lSel.getAttribute("href",2)
    ...
    var nl_Props = showModalDialog('<?=$spaw_dir?>dialogs/doc_library.php?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value + '&link=' + nProps.link, nProps, 'dialogHeight:420px; dialogWidth:400px; resizable:no; status:no');

//    if (nl_Props.actions == null || nl_Props.actions == "" ) {var lAction = '';}
//    else {var lAction=nl_Props.actions;}
    ...
    if (lAction == "InsertLink") {

   		targetWindow = nl_Props.target;

  		var linkSource = nl_Props.link;
	...
	
			{
				...
				var oNewLink = window.frames[editor+'_rEdit'].document.createElement("<A>");
	
				oNewSelection = window.frames[editor+'_rEdit'].document.selection.createRange();
	...
	
				{
					...
					selectedImage = window.frames[editor+'_rEdit'].document.selection.createRange()(0);
				...
				window.frames[editor+'_rEdit'].document.focus();
		...
		if (lAction == "RemoveLink") {
				...
				window.frames[editor+'_rEdit'].document.execCommand("Unlink");
...
// end document click
  ...
  function SPAW_image_popup_click(editor, sender)
  {
    ...
    if(imgSrc != null)    
	{
		...
		{
			// edit hyperlink
			a.href="#";	a.onclick="window.open('img_popup.php?img_url="+SPAW_stripAbsoluteUrlFromImg(imgSrc)+"','Image','width=500,height=300,scrollbars=no,toolbar=no,location=no,status=no,resizable=yes,screenX=120,screenY=100');return false;";
		}
		else
		{
			var a;
  ...
  function SPAW_stripAbsoluteUrlFromImg(url)
  {
   var adminDir = "/temp/admin";
	...
	{
		url = url.substr(chost.length);
		var hi = url.indexOf(adminDir,0);
	   if (hi != -1)
		  url = url.substr(hi+adminDir.length,url.length);
	}
	return(url);
  }
