...
$absimageurl = SITE_URL;//"http://www.identikid.com.au";
...
function checkOrderId($makenew, $newFromAdmin=false){
	...
	if(mysql_num_rows($result)>0 && !$newFromAdmin){
		...
		$query = "INSERT INTO orders (sessid, started, finished, customer, ordertype, currency) VALUES ('".$sessid."', '".date("Y-m-d H:i:s",time() + 7200)."', '".date("Y-m-d H:i:s",time() + 7200)."', ".mysql_insert_id().", '".$ordertype."', ".$currency.")";
