...
function addeditfundraiser(){
	...
	if($_POST["fname"]=="" || $_POST["idnumber"]==""){
		...
		if($add==true){
			$query = "INSERT INTO fundraisers (fname, idnumber, address, deladdress, contactname, contactphone, abn, abnexemption, gst, status, date_joined)"
			." VALUES ('".addslashes($_POST["fname"])."', '".$idnumber."', '".addslashes($_POST["address"])."', '".addslashes($_POST["deladdress"])
			."', '".$_POST["contactname"]."', '".$_POST["contactphone"]."', '".$_POST["abn"]."', '".$abnexemption."', ".$gst.", '".$_POST["stat"]."', '{$date_joined}')";
		}else{
			...
			.addslashes($_POST["contactphone"])."', abn='".$_POST["abn"]."', abnexemption='".$abnexemption."', gst=".$gst.", status='".$_POST["stat"]."', date_joined='{$date_joined}' WHERE id=".$_POST["id"];
		}
		$result = mysql_query($query);
		if(!$result) error_message(sql_error());
...
function updatereport(){
	...
	//header("location:fundraisers_showreports.php?".$sendString."&id=".$_GET["returnIds"]."&type=".$_GET["type"]);
}
