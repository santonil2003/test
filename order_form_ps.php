...
$postage[4] = $curTmp['postage2'];
$postage[5] = $curTmp['postage3'];
	 }
  ...
  $postage[4] = sprintf("%01.2f",($postage[4]*$cur['rate']));
  $postage[5] = sprintf("%01.2f",($postage[5]*$cur['rate']));
...
$subq = mysql_query("select shipping from product where id='".$qdata['type']."' LIMIT 1");

$subRow = mysql_fetch_array($subq);

if( $subRow['shipping']=='L' && $postageLevel <1 ){
$postageLevel = 0;

}elseif( $subRow['shipping']=='P' && $postageLevel <2 ){
$postageLevel = 1;
}elseif( $subRow['shipping']=='LP' && $postageLevel <3 ){
$postageLevel = 2;
}
...
if( $postageLevel==0 ){
$totalprice += $postage[0];
}elseif( $postageLevel==1 ){
$totalprice += $postage[4];
}elseif( $postageLevel==2 ){
$totalprice += $postage[5];
}
// get Voucher Details;
...
postageVals[0] = <? switch($postageLevel){
case 0:
echo $postage[0];
break;
case 1:
...
function validateRetrieve(){
	emailaddress = document.retrievedetails.emailadd.value;
	if(document.retrievedetails.firstname.value==""){
...
function validateForm(){
	emailaddress = document.form1.emailadd.value;
	if(document.form1.firstname.value==""){
		...
		return false;
	}else if(document.form1.address_cust.value==""){
		alert('You must enter your address');
		return false;
	}else if(document.form1.suburb_cust.value==""){
		alert('You must enter your suburb');
		return false;
	}else if(document.form1.postcode_cust.value==""){
		alert('You must enter your postcode');
		return false;
   }else if(document.form1.state_cust.value==""){
		alert('You must enter your state or territory');
		return false;
	}else if(document.form1.country_cust.value==""){
		alert('You must enter your country');
		return false;
	}else if(document.form1.country_cust.value=="Other" && document.form1.country_other_cust.value==""){
		alert('Your must enter a value for Country (Other)');
		return false;	
	}else if(document.form1.address.value==""){
		alert('You must enter a delivery address');
		return false;
	}else if(document.form1.suburb.value==""){
		...
		return false;
	}else if(document.form1.country.value=="Other" && document.form1.country_other.value==""){
		...
		alert('You must enter a valid email address');
		return false;
	}else if(document.form1.homephone.value==""){
		alert('You must enter home phone number');
		return false;
	}else if(document.form1.paymentmeth.value=="0"){
		alert('You must enter your payment method');
		return false;
	}else if(document.form1.referralcode.value!="" && document.form1.referralcode.value.length!=4){
		alert('The Fundraiser code must be exactly 4 digits long');
		return false;
	}else if($("paymentmeth").val() =="0"){
		  alert('You must select a Payment Method');
		  document.form1.paymentmeth.focus();
		  return false;
	}else if(document.form1.paymentmeth.value==6){
		if ((document.form1.vouchernumber[0].value == "") || (document.form1.vouchernumber[1].value == "") || (document.form1.vouchernumber[2].value == "") || (document.form1.vouchernumber[3].value == "")){
			...
			for (i = 0; i < 4; i++){
				vouchercode[i] = document.form1.vouchernumber[i].value
			}
			document.form1.vouchercode.value = vouchercode;
			document.form1.paymentmeth.disabled=false;
//			document.form1.action = "addphoneorder_voucherdetails.php";
                          ...
                          <tr> 
                            <td>
                                ...
                                <tr> 
                                  <td class="maintext"><img src="images/gen/spacer.gif" width="10" height="10"></td>
                                  <td><img src="images/gen/spacer.gif" width="10" height="10"></td>
                                </tr><form name="form1" method="post" action="submitorder_ps.php" onSubmit="return submitForm();">
                                <input type="hidden" name="orderid" value="<? echo $id;?>">
...
{
	if($_COOKIE['currency'] ==1 && $country_name=="Australia")
	{
		
		$SELECTED = "SELECTED";
	}
	elseif($_COOKIE['currency'] ==5 && $country_name=="New Zealand")
	{
		$SELECTED = "SELECTED";
	}
	else 
                               ...
                               <? //if ($_COOKIE['currency']==1) { ?>
                                ...
                                <tr> 
                                  <td height="23" valign="top" class="smalltext">Postage 
                                    &amp; Handling:</td>
                                  <td valign="top" class="smalltext"><? echo $cur['symbol'];?> 
                                    <input type="text" id="postage" name="postage" value="<? switch($postageLevel){
case 0:
echo sprintf("%01.2f", $postage[0]);
break;
case 1:
echo sprintf("%01.2f", $postage[4]);
break;
case 2:
echo sprintf("%01.2f", $postage[5]);
break;
default:
echo sprintf("%01.2f", $postage[0]);
break;
...
if($totalprice>0){
	if($_COOKIE["currency"]==1){?>
                                    <select name="paymentmeth" class="ordertext" id="paymentmeth" onChange="disableStuff();">
                                      ...
                                      <option style="display: block;" value="1"<? if($_POST['paymentmeth']==1){?> selected<? }?>>Pay by credit card Now</option>
        ...
        <br/><span style="font-size: 10px;"><i>Note:you can pay by paypal with a credit card . No paypal account required.</i></span>
                                    <?
	}else{
		?>
                                   <select name="paymentmeth" class="ordertext" id="paymentmeth" onChange="disableStuff();">
