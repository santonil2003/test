	...
	if (isset($_POST["action"])){
		...
		switch ($_POST["action"]){
			...
			case 'search':
				...
				$sql_search = "SELECT id, number, voucher_name, style, purchase_date, expiry_date, UNIX_TIMESTAMP(expiry_date) as u_time,  value, balance, currency, active FROM voucher ";
				switch ($id){
					case "number":
...
//echo '<br>' . $sql_search . '<br>';
			...
			case 'save':
				...
				// voucher name added, customer_id removed
				$sql_insert = "INSERT INTO voucher (number, purchase_date, expiry_date, style, value, balance, voucher_name, currency, active) VALUES ('" . $_POST["number"] . "', '" . date('Y-m-d', $purchase_timestamp) .  " 00:00:00', '" . date('Y-m-d', $expiry_timestamp) .  " 00:00:00', '" . $_POST["style"] . "', '" . $_POST["value"] . "', '" . $_POST["balance"] . "', '" . $_POST["voucher_name"] . "', '" .$_POST['currency']. "', '" .$_POST['active']. "')";
									...
									<td>
										<form name="results" method="post" action="vouchers.php">
											...
											<table width="100%" border="0" cellspacing="0" cellpadding="0" width=100%>
											  ...
											  <tr>
												  <td colspan="13"><img src="../images/spacer_trans.gif" height="5" width="1" border="0"></td>
											  </tr>
											  <tr bgcolor="#FFFFFF">
												  <td colspan="13"><img src="../images/spacer_trans.gif" height="2" width="1" border="0"></td>
											  </tr>
											  <tr>
												  <td colspan="13"><img src="../images/spacer_trans.gif" height="5" width="1" border="0"></td>
											  </tr>
...
// -->
												...
												<td><div align="left">&nbsp;Usable</div></td>
												<td>&nbsp;&nbsp;</td>
												<td>&nbsp;</td>
												<td>&nbsp;&nbsp;</td>
												<td>&nbsp;</td>
...
?>
														...
														<td>
															<div align="left">
														  		<input name="active" type="text" value="<?=$rs_results["active"]=='1' && $rs_results["u_time"] > time() && $rs_results["balance"] > 0 ?'Yes':'No' ?>" readonly="" size="3">
															</div>
														</td>
											  ...
											  <tr>
												  <td colspan="13"><div align="left">Click on the word 'edit' next to the result in order to view or update any of its details.</div></td>
											  </tr>
											</table>
										</form>
									</td>
...
//									$sql_edit = "SELECT voucher.id, voucher.number, voucher.purchase_date, voucher.expiry_date, voucher.style, voucher.value, voucher.balance, voucher.customer_id, voucher.recipient_id, voucher.currency, customers.firstname, customers.surname FROM voucher LEFT JOIN customers ON voucher.customer_id = customers.id WHERE (voucher.id = " . $id . ")";
										$sql_edit = "SELECT id, number, purchase_date, expiry_date, style, value, balance, voucher_name, currency, active from voucher WHERE id='". $id . "'";
											  ...
											  <tr>
												...
												<td>
													<select name="style" style="width: 100%">
														...
														<option value="online" <?php if ($rs_edit["style"] == "online"){ echo "selected"; } ?>>Online</option>
														?>
													</select>
												</td>
												<td>&nbsp;</td>
...
?>
 ...
 <tr>
												<td><div align="left">Active</div></td>
												<td align="left"><input type="checkbox" value="1" name="active" <?=$rs_edit["active"]=="1"?"CHECKED":"";?> >
