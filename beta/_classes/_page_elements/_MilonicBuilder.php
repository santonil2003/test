...
class MilonicBuilder
{
	...
	public $parent_str = '';
	public $section_seperator = '/';
	public $last_parent = 0;

	function buildMilonicMenu()
	{
...
//exit();	
	...
	{
		$full_file_name = SITE_DIR.'js/embed_menu.js'; 
...
//	function buildMenu($menu_name, $style, $items, $sections)
	function buildMenu($obj, $style)
	{
        ...
        $this->parent_str = str_replace(' ', '_',$obj->name);
		...
		foreach($items as $key=>$val)
		{
			...
			if($this->convertToMenuName($val->name)!="pages"){
				if($val->active){
					...
					{
						if($val->active){
						  $link_str=$this->parent_str.$this->section_seperator.str_replace(' ', '_',$val->title);
						  $js.=$js_start.'text='.$val->title.';url='.$link_str.$js_end."\n";
                    ...
                    */
					...
					elseif(strtolower(get_class($val))=='section' && $val->parent_id && !$mainmenu)
					{
					   ...
					   //$this->parent_str=str_replace(' ', '_',$val->name);
					   
					   $link_str=$this->parent_str.$this->section_seperator.str_replace(' ', '_',$val->name);

					   $this->last_parent = $val->parent_id;
					   
					   if( $val->default_page=='0' ) $js.=$js_start.'text='.$val->name.';'.$js_end."\n";
					   else $js.=$js_start.'text='.$val->name.';url='.$link_str.$js_end."\n"; 
					   
					   //if( $val->default_page=='0' ) $js.=$js_start.'text='.$val->name.';showmenu='.$this->convertToMenuName($val->name).';'.$js_end."\n";
					   //else $js.=$js_start.'text='.$val->name.';showmenu='.$this->convertToMenuName($val->name).';url='.$link_str.$js_end."\n"; 
